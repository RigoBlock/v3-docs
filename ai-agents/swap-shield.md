...
- Prevents users and agents from executing at clearly unfavorable prices (“much better than hoping the quote isn’t bad”).
- Works identically for human chat commands and fully autonomous strategies.
- Open-source and verifiable (see agentic-operator repo).
...
- Temporary disable: Use the `disable_swap_shield` tool (10-minute TTL).  
  ...
  Note: Because TWAP slices run on cron (min. 5 min intervals), disabling affects any slice within the TTL window.
...
### See also
- [NAV Shield](/ai-agents/nav-shield) (second layer — portfolio-level loss protection)
- [TWAP Order](/ai-agents/twap-order) (full safety stack example)
